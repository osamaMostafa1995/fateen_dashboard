<template>
  <CRow>
    <CCol :xs="12">
      <CCard class="mb-4">
        <CCardHeader>
          <strong>إضافة  لافتة رئيسية </strong>
        </CCardHeader>
  ...
  methods: {
    addBlog() {
      ...
      requestBody.append('name', this.blogTitle)
        requestBody.append('image', this.blogCoverImagePath)
      



      axios
        .post(`${baseUrl}/admin/brief-banner/add`, requestBody, config)
        .then((response) => {
